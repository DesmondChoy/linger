...
class ChatContextVarTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_uncertain_message_exposes_no_retrieval_scope(self) -> None:
        # A message with no catalog-metadata match and no explicit reader
        # confirmation resolves to "unknown" here, so boundary inference never
        # runs and no retrieval scope is exposed.
        seen: dict[str, object] = {}
