"""Tests for strict synthetic scenario validation."""
...
def test_cli_reports_valid_scenario(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    content_document = _content_document()
    backstory_bytes = _json_bytes(content_document)
    ground_truth_document = _ground_truth_document(content_document, backstory_bytes)
    backstory_path = tmp_path / "backstory.json"
    ...
    assert "SCENARIO_VALIDATION_OK=11 scenes,11 proposals" in capsys.readouterr().out
...
def test_rejects_content_hash_mismatch() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="backstory_sha256"):
        validate_scenario(
            content,
            ground_truth,
            backstory_bytes=backstory_bytes,
...
def test_rejects_exact_span_text_mismatch() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="span text mismatch"):
        validate_scenario(
            content,
            ground_truth,
            backstory_bytes=backstory_bytes,
...
def test_rejects_capture_mix_other_than_one_to_ten() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="1 capture_candidate"):
        validate_scenario(
            content,
            ground_truth,
            backstory_bytes=backstory_bytes,
...
def test_capture_scene_topology_is_validated_before_replay() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="fresh session"):
        validate_scenario(
            content,
            ground_truth,
            backstory_bytes=backstory_bytes,
...
def test_rejects_generic_spans_in_capture_run() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="generic exact_spans"):
        validate_scenario(
            content,
            ground_truth,
            backstory_bytes=backstory_bytes,
...
def test_pairing_checks_declared_matches_and_differences() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="must match on line_text"):
        validate_scenario(
            content,
            broken,
            backstory_bytes=backstory_bytes,
...
def test_repository_evidence_resolves_hash_and_exact_span(tmp_path: Path) -> None:
    ...
    _, broken = _validated_models(content_document, broken_ground_truth)
    with pytest.raises(ScenarioValidationError, match="source_sha256"):
        validate_scenario(
            content,
            broken,
            backstory_bytes=backstory_bytes,
