...
def test_rejects_wrong_prop_count() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="requires 11 Props"):
        _validate(content, ground_truth)
...
def test_rejects_missing_prop_relevance_judgment() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="judgment for every Prop"):
        _validate(content, ground_truth)
...
def test_rejects_wrong_relevance_mix() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="retrieval Prop mixes"):
        _validate(content, ground_truth)
...
def test_rejects_different_prop_banks() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="same Prop bank"):
        _validate(content, ground_truth)
...
def test_rejects_inactive_prop() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="requires Prop .* to be active"):
        _validate(content, ground_truth)
...
def test_rejects_prop_evidence_for_a_distractor() -> None:
    ...
    with pytest.raises(ScenarioValidationError, match="exactly match"):
        _validate(content, ground_truth)
