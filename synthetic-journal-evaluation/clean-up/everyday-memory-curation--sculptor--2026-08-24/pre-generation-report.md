...
The current implementation is **insufficient** for the complete selected plan. Sculptor can produce validated proposals for the five accepted behaviors, but no scenario adapter can replay Props through that production boundary, and the scenario Ground truth cannot yet express typed curation expectations. Do not run the detached prompt until the approved gaps below are implemented and tested.

| Required Scene | Target behavior | Status | Evidence or gap |
|---|---|---|---|
| Exact duplicate | Link two identical Props without changing either | partially runnable | `propose_curation` and `sculptor-exact-duplicate-v1` cover the behavior; scenario-to-Sculptor adapter and typed Ground truth are missing |
| Paraphrased duplicate | Link differently worded Props that express the same durable memory | partially runnable | `sculptor-paraphrased-duplicate-v1` and the hard grader exist; replay and scenario mapping are missing |
| Meaningful update and noise | Propose a supported derived summary from fragments and a later refinement while excluding noise | partially runnable | `sculptor-noisy-memory-summary-v1` covers summary constraints; generated-scenario semantic criteria and replay are missing |
| Related records | Propose a topic group for related, distinct Props and leave a distractor outside it | partially runnable | `sculptor-related-topic-group-v1` covers the action; generated-scenario source mapping and replay are missing |
| Unrelated comparison | Return no change for superficial overlap | partially runnable | `sculptor-superficial-similarity-no-change-v1` covers abstention; generated-scenario comparison and replay are missing |
...
The design uses the seven canonical nouns in [specification Section 7.2.1](../../../docs/specification.md#721-canonical-vocabulary). It is memory-only; book material would add no useful evidence.
...
- evals/synthetic_journals/validate_scenario.py deterministically validates those curation expectations, all permitted Scene Prop IDs, active lifecycle state, exact spans, and Prop evidence.
- The approved scenario-to-Sculptor replay adapter exists, calls production propose_curation, records full-deployment and objective-execution identities, proves source immutability, and has focused passing tests.
- SCENARIO_DIRECTORY is synthetic-journal-evaluation/clean-up/everyday-memory-curation--sculptor--2026-08-24, contains only pre-generation-report.md, and has no backstory.json or ground-truth.json.
...
Create exactly one memory-only synthetic-journal scenario for the bounded_memory_curation Objective. Use the current scenario models and validator unchanged at invocation time. Write only SCENARIO_DIRECTORY/backstory.json and the separate proposed Ground truth file SCENARIO_DIRECTORY/ground-truth.json. Do not invoke Linger, run a model evaluation, adopt labels, grade output, create replay data, or write any other file.

For SCENARIO_DIRECTORY/backstory.json:
...
For SCENARIO_DIRECTORY/ground-truth.json:
