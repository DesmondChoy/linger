...
Use the seven [canonical nouns](../../../docs/specification.md#721-canonical-vocabulary) as follows.

| Noun | Application |
|---|---|
| Objective | The two confirmed catalog entries govern one coherent three-Scene scenario; no run configuration applies. |
...
SCENARIO_DIRECTORY=synthetic-journal-evaluation/scenarios/alice-caterpillar-grounding-and-spoilers--muse-librarian-provenance--2026-08-29

You have read-only access to the current checkout. Inspect only data/corpus/, evals/synthetic_journals/models.py, and evals/synthetic_journals/validate_scenario.py at invocation time. Discover the available work, immutable version, ordered structure, and exact evidence from data/corpus/; do not rely on a book fact from an earlier report.

Use evals/synthetic_journals/models.py unchanged as the Backstory and proposed Ground truth contract. Use evals/synthetic_journals/validate_scenario.py unchanged as the deterministic scenario validator. Write exactly SCENARIO_DIRECTORY/backstory.json and SCENARIO_DIRECTORY/ground-truth.json. Do not write any other file.
...
Create a separate ground truth file at SCENARIO_DIRECTORY/ground-truth.json with ground_truth_status "proposed" and the SHA-256 of the exact SCENARIO_DIRECTORY/backstory.json bytes. Add one proposed GroundTruthProposal for every Scene/Objective pair required by the Scene graph. These are candidate labels, not adopted labels.
...
> **Human decision required:** Approve the target design and runnable prompt, request revisions, or abandon this plan. Approval authorizes generation of the two proposed scenario files, not adoption or evaluation.
