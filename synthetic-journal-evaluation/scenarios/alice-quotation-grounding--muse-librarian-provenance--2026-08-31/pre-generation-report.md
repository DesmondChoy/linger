...
| S1/S2 pairing | Both Scenes share Props and differ on Line text | `runnable` | `validate_scenario.py:547` `_validate_pairing` resolves `match_fields` and `difference_fields` |
...
`tests/test_synthetic_reflection_scenario.py`,
...
The seven canonical nouns come from
...
**Observed.** Reusable `evals/` assets: `models.py` and `validate_scenario.py`,
...
2026-08-31T232340+0800, dirty only in an untracked earlier scenario directory and
...
- evals/synthetic_journals/models.py and
  evals/synthetic_journals/validate_scenario.py exist and are the schema and
...
TASK
Write one synthetic evaluation scenario for the single Objective
...
exact bytes you wrote to SCENARIO_DIRECTORY/backstory.json. Include exactly one
...
Run from the repository root and fix anything it reports:
  uv run python -m evals.synthetic_journals.validate_scenario \
    SCENARIO_DIRECTORY/backstory.json SCENARIO_DIRECTORY/ground-truth.json
```

## Ground truth lifecycle

The generator **proposes**. It writes `ground-truth.json` beside
`backstory.json`, anchoring every label to identifiers in the scenario, with
exact code-point spans, corpus-resolvable evidence, and the S1/S2 pairing.

Repository code **validates objective facts only**.
`evals/synthetic_journals/validate_scenario.py` checks that `backstory_sha256`
...
"end-to-end flow verification" (p. 11). This scenario supplies exactly that — a
