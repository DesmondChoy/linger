...
The current implementation is **sufficient** for the selected design and a separately approved generation step. The scenario validator, human review, and replay now use the same book answer key. This establishes runnable evaluation infrastructure, not proven model accuracy.
...
| Support agreement | Catalog, specification, review dispatch, and README agree on exact supported selections. Old book scenarios remain historical, obsolete inputs. |
...
SCENARIO_DIRECTORY=synthetic-journal-evaluation/scenarios/alice-pigeon-grounding-and-spoilers--muse-librarian-provenance--2026-09-03
...
Write SCENARIO_DIRECTORY/backstory.json and compute the SHA-256 of its exact bytes.

Write the separate ground truth file at SCENARIO_DIRECTORY/ground-truth.json with ground_truth_status set to proposed. Produce four proposed Ground truth judgments, one per Scene-Objective pair, using only the finalized repository contract.
...
Run evals/synthetic_journals/validate_scenario.py against the two sibling files. Correct only those files and rerun until schema, Backstory hash, references, ordering, spans, pairings, corpus evidence, and Objective facts validate. Stop if a failure requires repository code changes.
...
> **Human decision required:** Approve the target design and generator prompt, request revisions, or abandon the scenario. No generation, adoption, or provider-backed replay has been performed.
