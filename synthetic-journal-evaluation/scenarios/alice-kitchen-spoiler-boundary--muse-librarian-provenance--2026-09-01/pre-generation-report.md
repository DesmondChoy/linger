...
compare correctly. The scenario at `scenarios/alice-quotation-grounding--muse-librarian-provenance--2026-08-31/` runs the
...
`test_boundary_observability.py`, `test_synthetic_reflection_scenario.py`,
...
`5f9a6cdc6a0d03a7d09708164d6578e4bcea222a`, dirty only with untracked scenario
...
TASK
Create one synthetic evaluation scenario for the Objective
...
where SCENARIO_DIRECTORY is
synthetic-journal-evaluation/scenarios/alice-kitchen-spoiler-boundary--muse-librarian-provenance--2026-09-01.
...
this scenario; its `ground_truth_status` is "proposed" and its
`backstory_sha256` is the SHA-256 of the exact bytes you wrote to
backstory.json. Validate your work by running
evals/synthetic_journals/validate_scenario.py against both files and fixing every
failure it reports.

PERMITTED REPOSITORY PATHS (read-only)
  data/corpus/                                    the book corpus
  evals/synthetic_journals/models.py              the scenario contract
  evals/synthetic_journals/validate_scenario.py    the deterministic validator
...
[validate_scenario.py](../../../evals/synthetic_journals/validate_scenario.py)
then checks objective facts, failing as `SCENARIO_VALIDATION_FAILED`. None of it
...
relevant AI security tests." A validated scenario replayed through the production
