...
*The outer path is Muse answering directly when no grounding is needed.*
...
| **Agent autonomy & orchestration** | Framework-native orchestration (Pi in TypeScript, PydanticAI in Python) implementing plan → act → check → refine: agents select tools, respond to incomplete evidence, decline unsafe actions, and coordinate through typed contracts carrying bounded, role-specific context. |
...
| 8 | **Orchestration & integration** (Pi/PydanticAI agent assembly, typed tool contracts, end-to-end flow, failure handling) | M1 | 6 |
