...
source. Allow `restore_to_retrieval` only when the named original has
`retrieval_state="tombstoned"` and restoration is consistent with its source
text. Return an `invalid_restore` finding when the named original is active.
