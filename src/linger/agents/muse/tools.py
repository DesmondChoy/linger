...
async def serendipity_explore(
    ...
    Use `gather_sources` when the reader names the specific sources to consider
    together (books or scenes, a named public text, their own earlier writing)
    and asks how they relate or whether they support a conclusion. It returns
    one bundle with the records for every named source it found, and names any
    it could not find; you write the comparison.
    Use `find_connection` when the reader asks whether anything connects without
    naming the sources, or when a connection to a confirmed book or wider public
    resonance could deepen reflection, including when the supported answer may
    be a decline.
    ...
    compares a shortlist, and returns a validated proposal, recall, bundle, or
    decline together with its request-local evidence. Use `get_recommendation` when the reader
    explicitly asks for an essay, artwork, song, thinker, or other outside
    source; use `gather_sources` for named sources; use `find_connection` for
    open-ended discovery or an optional reflective resonance. Never
