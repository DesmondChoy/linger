...
function InspectorContent({ selection, scene, objective }: { selection: InspectorSelection; scene: Scene; objective: Objective | undefined }) {
  switch (selection.kind) {
    ...
    case 'criteria':
      if (!objective) return <p className="drawer-intro">Objective criteria are available in the scenario explorer.</p>
      return <>
    ...
    case 'history':
      if (!objective) return <p className="drawer-intro">Archived packages are available in the scenario explorer.</p>
      return <History objective={objective} />
    case 'about': return <>
...
export function Inspector({ selection, scene, objective, onClose }: { selection: InspectorSelection | null; scene: Scene; objective?: Objective; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="map-inspector" aria-labelledby="inspector-title" onCancel={onClose} onClose={onClose} onClick={event => { if (event.target === event.currentTarget) { const bounds = event.currentTarget.getBoundingClientRect(); if (event.clientX < bounds.left || event.clientX > bounds.right || event.clientY < bounds.top || event.clientY > bounds.bottom) onClose() } }}>
