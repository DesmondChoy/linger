...
class ScenarioValidationError(ValueError):
    """One or more deterministic scenario checks failed."""
...
def validate_scenario_files(
    ...
    *,
    ...
    """Load two JSON files and validate their complete scenario graph."""
    ...
    try:
        ...
        raise ScenarioValidationError([_format_pydantic_error(error)]) from error
...
def validate_scenario(
    ...
    *,
    ...
    for facts in ground_truth.book_scene_facts:
        scene = scenes.get(facts.scene_id)
        if scene is None:
            continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (OSError, ScenarioValidationError) as error:
        print(f"SCENARIO_VALIDATION_FAILED={error}", file=sys.stderr)
        return 1
    print(
        "SCENARIO_VALIDATION_OK="
