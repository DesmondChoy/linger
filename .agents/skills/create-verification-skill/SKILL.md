...
metadata:
  short-description: generate a project-local verification skill and feature map
---
...
If the checkout does not build or start, diagnose and report the blocker. Fix product code only when the user's request authorizes that additional implementation. Do not write a verification skill against steps you could not prove.

## 2. Generate the skill

Use **skill-creator** to write `.agents/skills/verify-<app>/SKILL.md` with supported YAML frontmatter and these grounded sections:
...
Create `.agents/skills/verify-<app>/features/README.md` plus one file per user-facing feature you can identify, starting with three to five. Follow [`references/feature-map-example/`](references/feature-map-example/).
