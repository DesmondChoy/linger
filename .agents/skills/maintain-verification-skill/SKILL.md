...
**Runtime note.** The parallel per-feature source readers below map to the runtime's subagent tool (`spawn_agent` on Codex, `Agent` on Claude Code) fan-out, and the project-local skill lives under `.agents/skills/`. See [`runtime-adaptation.md`](../poteto-mode/references/runtime-adaptation.md).
