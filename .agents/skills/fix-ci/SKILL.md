...
metadata:
  short-description: find failing PR checks, inspect logs, apply focused fixes
---
...
4. If the user authorized code changes and pushes, run repository quality gates, commit the focused fix, follow the repository's Beads sync rule, push, and re-check. Otherwise stop after diagnosis or the verified local fix and report the commands that remain.
