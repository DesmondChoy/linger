...
The orchestrator passes this file through to every parallel candidate runner during Phase B and fills in the variable inputs around it: the task, the Phase A grounding artifacts, the isolated working directory, and the path to write outputs. In Linger, never create git worktrees; use a per-runner directory under `/tmp` or another disjoint output directory. What matters is independence between candidates.
...
You are one of several independent runners. Produce the best design for your assigned lens; don't hedge against the others. Runners may share the parent model unless project-local overrides select different valid models. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
