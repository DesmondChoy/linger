...
3. Migrate one component at a time. Independent components may run in waves of owners within the runtime's concurrency limit (three on Codex). In Linger, never create worktrees; assign disjoint paths in the shared checkout and serialize any shared primitive migration first (the **separate-before-serializing-shared-state** principle skill).
4. Verify each component against its baseline via image diff on the matching surface. Run CLIs and TUIs directly; use the runtime's browser, computer-use, test, or runtime tool for UIs. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Repeat locally or use the runtime's recurring mechanism when an unattended loop is authorized until the diff is zero.
