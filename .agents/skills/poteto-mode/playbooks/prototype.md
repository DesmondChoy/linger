...
2. Gather references when the design space is open. Use the request and available evidence to choose a promising direction and build a concrete sketch the user can react to. Ask first only if a missing preference materially determines the result or the user explicitly requested a checkpoint. Skip research when the direction is set.
