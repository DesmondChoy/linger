...
2. Don't cross an irreversible line to pause. Do not open a PR, commit, or push unless the user already authorized that action.
3. Make the work durable. When commit authorization exists, commit uncommitted edits as one clear `wip:` commit on the current branch so nothing is lost. If the tree is broken, say so in the commit body in one line. Without commit authorization, preserve the working tree, enumerate its changed paths, and rely on the resume note in step 4.
