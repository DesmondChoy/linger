---
name: autoreview
description: "Run isolated, structured code review over local, branch, or commit diffs before commit, PR update, release, or shipping. Use when the user asks for autoreview, Codex review, Claude review, Amp review, Pi review, Kimi review, a second-model review, or a closeout review after non-trivial code changes. Codex is the default engine."
---

# Auto Review

Run the bundled structured review helper as a closeout check. It reviews a validated change bundle with an isolated reviewer process; it does not replace direct inspection, focused tests, or user approval for external actions.
...
For user-visible behavior, also exercise the running product against its behavior contract. Autoreview is source-aware and judges the change bundle; a clean result is not proof that a UI, CLI, API, or generated artifact works from the user's perspective.
...
- If automation merged the blamed PR, identify the human trigger when practical by checking the PR timeline, comments, labels, or public PR page. Report `automerge triggered by @login`; if no trigger is traceable, say it is unknown.
...
This is a project-local Linger skill. Set its script paths once, then use `"$AUTOREVIEW"` and `"$AUTOREVIEW_HARNESS"` below:
...
On Windows, the default `--parallel-tests` shell preserves `cmd.exe` semantics.
Use `--parallel-tests-shell powershell` or `--parallel-tests-shell pwsh` for
PowerShell-specific commands. Parallel tests inherit only a small allowlist of
ordinary OS, CI, and toolchain variables and run with isolated home/config
directories. Put additional non-secret controls directly in the test command;
never put secrets in it because the command is printed before execution.
...
Adapted for Linger from [openclaw/agent-skills](https://github.com/openclaw/agent-skills/tree/main/skills/autoreview). The bundled runtime retains its upstream MIT license in `LICENSE`.
