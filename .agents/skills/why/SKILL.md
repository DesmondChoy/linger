...
metadata:
  short-description: investigate why something was built this way (parallel multi-MCP evidence)
---
...
After investigator waves finish, spawn one read-only synthesizer when a collaboration slot is available. Omit its model override unless `.agents/pstack-models.md` contains a valid `why synthesizer` value. Connector access permits citation checks, not writes.
