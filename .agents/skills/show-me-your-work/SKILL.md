...
metadata:
  short-description: log decisions to a reviewable tsv decision trail
---
...
2026-05-24T12:30:00Z	widget	threw out a helper's work because its screenshots were blank	checked the real files instead of trusting its summary	candidate output discarded	reverted, tightened the instructions for next time
```
...
Before handing back a material audit trail, spawn one read-only collaboration agent when the active skill and runtime allow delegation. Omit the model override unless `.agents/pstack-models.md` names a valid alternative. The reviewer reads the trail and active task evidence, then flags what the user should inspect. If collaboration is unavailable, perform the same audit yourself and disclose that limitation.
...
Every reply for a run that produced a material trail ends with an "Attention" section. State whether a collaboration reviewer or self-review performed the audit, then list specific rows or moments to inspect. "No flags" is a valid result.
